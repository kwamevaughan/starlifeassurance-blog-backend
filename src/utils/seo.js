export const calculateSEOScore = (formData, editorContent) => {
  ...
  if (
    ...
    score++;
  if (
    formData.slug?.toLowerCase().includes(formData.focus_keyword?.toLowerCase())
  )
    score++;
  if (
  ...
  if (
    headings.some((h) =>
      h.toLowerCase().includes(formData.focus_keyword?.toLowerCase())
    )
  )
    score++;
  ...
  if (
    altTexts.some((alt) =>
      alt.toLowerCase().includes(formData.focus_keyword?.toLowerCase())
    )
  )
    score++;
  const density = editorContent
    ? ((
        editorContent
          .toLowerCase()
          .match(new RegExp(formData.focus_keyword?.toLowerCase(), "g")) || []
      ).length /
        (editorContent.split(/\s+/).filter(Boolean).length || 1)) *
      100
    : 0;
  ...
  // Readability checks
  const firstWords = formData.article_name
    ?.toLowerCase()
    .split(" ")
    .slice(0, 3)
    .join(" ");
  ...
  if (paragraphs.every((p) => p.split(/\s+/).filter(Boolean).length <= 150))
    score++;
...
export const getScoreColor = (score, mode) => {
  ...
  if (score >= 60)
    return mode === "dark" ? "text-orange-400" : "text-orange-600";
